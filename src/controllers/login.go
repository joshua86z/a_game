...
func (this *Login) Login(uid int64, commandRequest *protodata.CommandRequest) (string, error) {
	...
	models.InsertSign(uid)

	response := &protodata.LoginResponse{TokenStr: proto.String(token)}
	return ReturnStr(commandRequest, 1, response), nil
}

func otherLogin(platId int, otherId string, session string, sign string, otherData string) bool {
	...
	Lua.L.DoString(fmt.Sprintf("isLogin = isLogin('%s', '%s', '%s', '%s', '%s')", otherId, session, sign, otherData, body))
